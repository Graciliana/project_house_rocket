...
# ============
st.set_page_config(
    page_title="Dashboard - House Rocket", page_icon=":cityscape:", layout="wide"
)

# Introduction page
# ===================
st.markdown(
    "<h1 style='text-align: center; color: black;'>Project House Rocket</h1>",
...
def portfolio_density(data, geofile):
    ...
    st.markdown(
        "In this step we will see how the House Rocket dataset it is distributed by zipcode and by region."
    )
    ...
    density_map = folium.Map(
        location=[data["lat"].mean(), data["long"].mean()], default_zoom_start=10
    )
    ...
    for name, row in data.iterrows():
        folium.Marker(
            [row["lat"], row["long"]],
            popup="Sold R${0} on: {1}, Features:{2} m2, {3} bedrooms, {4} bathrooms,zipcode:{5} County: {6}, Town:{7}".format(
                row["price"],
                row["date"],
                row["sqft_living"],
                row["bedrooms"],
    ...
    df2 = np.round(
        data[["price_m2", "zipcode"]].groupby("zipcode").mean(), 1
    ).reset_index()
    df3 = np.round(
        data[["price_m2", "zipcode"]].groupby("zipcode").std(), 1
    ).reset_index()
    df4 = np.round(
        data[["bedrooms", "zipcode"]].groupby("zipcode").mean(), 1
    ).reset_index()
    df5 = np.round(
        data[["bathrooms_real", "zipcode"]].groupby("zipcode").mean(), 1
    ).reset_index()
...
def region_town(data):
    ...
    st.markdown(
        "In this step I collected the data from an API, so that the user can identify the median by region."
    )
    ...
    fig = px.bar(
        aux03, y="price", x="town", width=500, height=500, template="plotly_white"
    )
    fig.update_layout(
        uniformtext_minsize=12,
        uniformtext_mode="hide",
        xaxis={"title": "city"},
        yaxis={"title": "quantity of houses"},
        xaxis_tickangle=-45,
    )
    ...
    fig = px.bar(
        aux04, y="price", x="town", width=500, height=500, template="plotly_white"
    )
    fig.update_layout(
        uniformtext_minsize=12,
        uniformtext_mode="hide",
...
def season_sale(data):
    ...
    st.markdown(
        "In this step an analysis of the quantity of sales per season and we demonstrate median of monthly sales"
    )
    ...
    # plot the graph
    fig = px.bar(
        aux, y="season", x="price", width=600, height=500, template="plotly_white"
    )
    fig.update_layout(
        uniformtext_minsize=12,
        xaxis={"title": "quantity of houses"},
    ...
    fig.update_layout(
        uniformtext_minsize=12, uniformtext_mode="show", yaxis={"title": "median price"}
    )
...
def sales_simulation(data):
    ...
    st.markdown(
        "In this step you can simulate the best and worst scenario for selling a house. Which we calculate from the region and the choice of season."
    )
    ...
    # Filters
    f_zipcode = st.sidebar.selectbox(
        "Select Zipcode", sorted(set(data["zipcode"].unique()))
    )
    f_price = st.sidebar.number_input("Select Price")
    f_season = st.sidebar.selectbox(
        "Select Season", options=["summer", "autumn", "winter", "spring"]
    )
    ...
    fig = px.bar(
        y=[f_price, price_median, best_scene, worst_scene],
        x=["Price", "Price Median", "Best Scene", "Worst Scene"],
        width=600,
        height=500,
        template="plotly_white",
        labels={"x": "", "y": ""},
    ...
    fig.update_layout(
        uniformtext_minsize=12, uniformtext_mode="hide", xaxis_tickangle=-45
    )
    fig.update_traces(marker_color="rgb(0,0,139)")
